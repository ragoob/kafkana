...
export class KafkaMonitorService   {
    ...
    getMessages(topic: string, clusterId: string,size?: number): Promise<Message[]>{
        const url = size ? `${this.baseUrl}/monitoring/${clusterId}/getLatestMessages/${topic}?size=${size}` : `${this.baseUrl}/monitoring/${clusterId}/getLatestMessages/${topic}`;
    ...
    getLatestMessages(topic: string, clusterId: string): Promise<Message[]> {
        return this.http.get<Message[]>(`${this.baseUrl}/monitoring/${clusterId}/getLatestMessages/${topic}`)
            .pipe(map(m => {
                m.forEach(msg => {
                    if (msg) {
                        try {
